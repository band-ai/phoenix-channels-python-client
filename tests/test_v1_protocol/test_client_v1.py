...
from .conftest import FakePhoenixServerV1


@pytest.mark.asyncio
async def test_subscribe_to_topic_succeeds_when_subscribing_to_valid_topic(
    phoenix_server: FakePhoenixServerV1,
):
...
async def test_callback_receives_message_when_server_sends_message_to_subscribed_topic(
    ...
    async with PHXChannelsClient(
        ...
        await phoenix_server.simulate_server_event(
            "test-topic", "new_message", test_payload
        )
    ...
    async with PHXChannelsClient(
        ...
        event_tasks = [
            phoenix_server.simulate_server_event(
                "test-topic", "burst_event", {"event_id": i}
            )
            for i in range(
...
@pytest.mark.asyncio
async def test_two_topics_with_different_callbacks(phoenix_server: FakePhoenixServerV1):
